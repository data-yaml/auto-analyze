...
export async function handler(event: any, context: any): Promise<any> {
    const AWS_ACCOUNT_ID = (await new AWS.STS().getCallerIdentity().promise()).Account!;
    ...
    if (omics_workflowId === UPSTREAM_WORKFLOW_ID) {
        console.info(`Omics Workflow ID: ${omics_workflowId} matched, continue processing`);
    } else {
        console.info(`ERROR! Expected input from workflow (${UPSTREAM_WORKFLOW_ID}), but received input from workflow (${omics_workflowId}) `);
        return {
            'statusCode': 200,
    ...
    do {
        const list_objects_response: any = await s3.listObjectsV2({
            Bucket: s3bucket,
            Prefix: s3key,
    ...
    try {
        const run = await omics.startRun({
        ...
        const run_id = run.id;
        console.info(`Successfully started HealthOmics Run ID: ${run_id} for sample: ${sample_name}`);
    } catch (e: any) {
