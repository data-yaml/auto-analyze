...
class omics_workflow_Stack(Stack):
    def __init__(self, scope: Construct, construct_id: str, config, **kwargs) -> None:
        ...
        sns_topic = sns.Topic(
            self,
            f"{APP_NAME}_workflow_status_topic",
        ...
        rule_workflow_status_topic = events.Rule(
            self,
            f"{APP_NAME}_rule_workflow_status_topic",
        ...
        omics_s3_write_policy = iam.PolicyStatement(
            ...
            resources=[bucket_output.bucket_arn, bucket_output.bucket_arn + "/*"],
        )
        ...
        lambda_iam_passrole_policy = iam.PolicyStatement(
            actions=["iam:PassRole"], resources=[omics_role.role_arn]
        )
        ...
        workflow_zip_asset = s3_assets.Asset(
            self, PRIVATE_WORKFLOW_NAME, path=private_workflow_dir + "nextflow/"
        )
        ...
        private_workflow_cfn = omics.CfnWorkflow(
            self,
            f"{APP_NAME}-workflow-{PRIVATE_WORKFLOW_NAME}",
        ...
        initial_workflow_lambda = lambda_.Function(
            self,
            f"{APP_NAME}_initial_workflow_lambda",
        ...
        second_workflow_lambda = lambda_.Function(
            self,
            f"{APP_NAME}_post_initial_workflow_lambda",
            runtime=lambda_.Runtime.PYTHON_3_8,
        ...
        rule_second_workflow_lambda = events.Rule(
            self,
            f"{APP_NAME}_rule_second_workflow_lambda",
            event_pattern=events.EventPattern(
                source=["aws.omics"],
                detail_type=["Run Status Change"],
        ...
        rule_second_workflow_lambda.add_target(
            events_targets.LambdaFunction(second_workflow_lambda)
        )
